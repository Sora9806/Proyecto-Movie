...
function cargarDetalleCartelera(){
    ...
    const piePagina =  `\nRecordá que podés visitar las secciones:\n\n• En Cartelera (/en-cartelera)\n• Más Votadas (/mas-votadas)\n• Sucursales (/sucursales)\n• Contacto (/contacto)\n• Preguntas Frecuentes (/preguntas-frecuentes)`;
    return totalPeli + titulosConResenias + piePagina;
}
function cargarMasVotadas(){
	let promedioPelis = 0;
    movies.forEach(elemento => {promedioPelis += elemento.vote_average});
    let promedio = Math.round(promedioPelis/movies.length);
    let masValoradas = movies.filter(movie => movie.vote_average >= 7);
    let totalPelisRating ="Total Peliculas mas Valoradas : "+  masValoradas.length + '\n\n';
    let masValuadas = ' \n';
    for (let pelis of masValoradas) {
		masValuadas += `${pelis.title}\n${pelis.vote_average}\n${pelis.overview}\n\n`;
	};
	const piePagina =  `\nRecordá que podés visitar las secciones:\n\n• En Cartelera (/en-cartelera)\n• Más Votadas (/mas-votadas)\n• Sucursales (/sucursales)\n• Contacto (/contacto)\n• Preguntas Frecuentes (/preguntas-frecuentes)`;
	return totalPelisRating + masValuadas + promedio +  piePagina;
	// res.end(`${tituloMasVotadas}\n\n\n ${totalPeli}\n\n\n${totalPelisRating}\nRating promedio: ${promedio}\n${masValuadas}`);
  };
  function cargarSucursales(){
    const tituloSucursales = 'Nuestras Salas.\n';
    const totalSucursales = '\nTotal de salas​: ​(' + theaters.length + ')';
    const listadoSucursales = '\n\nListados de salas:'
    const porCadaSAla = theaters.map(theaters =>"\n\n Nombre: " +theaters.name + "\n Direccion: " + theaters.address + "\n Descripción: "+ theaters.description);
    return tituloSucursales + totalSucursales + listadoSucursales + porCadaSAla;
};
function cargarPreguntas(){
	const tituloPreguntas = 'Preguntas Frecuentes.\n';
	let totalPreguntas = "";
	totalPreguntas += `\n\tTotal de preguntas: ${faqs.length}\n\n\n`;
	for (let pregunta of faqs){
				totalPreguntas += `\n\t ${pregunta.faq_title} \n\n\t ${pregunta.faq_answer} \n\n\n`
			}
return tituloPreguntas + totalPreguntas
}
/** 
...
const server = http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
            ...
            res.write('Home\n' + detalleHome);
            //usamos la función write para enviar el texto requerido en el cuerpo de la respuesta al browser.
        ...
        case '/en-cartelera':
			...
			res.end();
            //res.end(`${tituloCartelera}\n\n\nTotal de películas: ${totalPelis}\n\n\n${titulosConResenias}`);
            break;
		case '/mas-votadas':
			let detalleMasVotadas = cargarMasVotadas();
			res.write("Titulos Mas Votados\n" + detalleMasVotadas)
			res.end();
			break;
		case '/sucursales':
			let detalleSucursales = cargarSucursales();
			res.write(detalleSucursales);
			res.end();
			break;
		case '/contacto':
			...
			res.end(infoContacto);
			break;
		case '/preguntas-frecuentes':
			let preguntasFrecuentes = cargarPreguntas();
			res.write(preguntasFrecuentes)
			res.end();
			break;
		default:
